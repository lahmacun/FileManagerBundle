...
class File
{
     ...
     * File constructor.
     */
    public function __construct(private SplFileInfo $file, private TranslatorInterface $translator, private FileTypeService $fileTypeService, private FileManager $fileManager, private ParameterBagInterface $parameterBag)
    {
    ...
    public function getAttribut(): ?string
    {
        if ($this->fileManager->getModule()) {
            ...
            if ('file' === $this->file->getType()) {
                ...
                $attr .= " data-relative-path=\"{$relativePath}\"";
            }
    ...
    {
        $publicDir = $this->parameterBag->get('artgris_file_manager')['web_dir'];

        return str_replace($publicDir, '', $this->file->getPathname());
    }
}
